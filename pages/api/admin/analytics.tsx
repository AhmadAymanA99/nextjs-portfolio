...
export default async function handler(req, res) {
  ...
  try {
    ...
    const dateFilter = days ? sql`WHERE timestamp >= NOW() - INTERVAL '${days} days'` : sql``

    const totalViewsResult = await sql`SELECT COUNT(*) as count FROM page_views ${dateFilter}`
    ...
    const deviceDistribution = {
      mobile: Math.round(
        ((deviceStatsResult.find((d) => d.device_type === 'mobile')?.count || 0) / totalViews) *
          100,
      ),
      desktop: Math.round(
        ((deviceStatsResult.find((d) => d.device_type === 'desktop')?.count || 0) / totalViews) *
          100,
      ),
    }
